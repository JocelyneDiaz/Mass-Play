...
def create_level(level):
    room_number = int(rs.RealBox("Please imagine the kinds of rooms that you would like on this level (ie Kitchens, Bathrooms, Excercise Rooms). How many of these rooms are there?"))
    scale_factor = 1.25
    rooms = []
    Mass_Play = rs.MessageBox("You are going to start selecting the sizes of square feet of the number of rooms you input, would you like to proceed?", 4, 'Mass Play')
    for j in range(room_number):
        Radius = int(rs.RealBox("How many Square feet would you like one of the rooms to be ?"))
        point = rs.GetPoint("Select a point on the site to place a room")
...
def LinearColor(R,G,B,R2,G2,B2,ColorPercentage):
    ...
    #The base color that will be altered by the percentage should be entered in the second R2,G2,B2 value
...
def main():
    ...
    Mass_Play = rs.MessageBox("This program produces mass schemes, would you like to proceed?", 4, 'Mass Play')
    level_number = rs.RealBox ("How many levels should this building have?")
    level_height = 10
    top_floor_level = level_height* int(level_number)
